...
public class Plugins {
    ...
    public static void do_action(String hook, Object arg0, Object arg1, Object arg2) {
        do_action(hook, new Object[] { arg0, arg1, arg2 } );
    }
    
    public static void do_action(String hook, Object arg0, Object arg1, Object arg2, Object arg3) {
    ...
    public static void add_action(String hook, Runnable callback, int priority, int num_args) {
        if (actions.containsKey(hook)) {
            actions.get(hook).add(new ActionHook(callback, priority, num_args));
        } else {
    ...
    public static boolean has_action(String hook, Runnable callback) {
        return (actions.containsKey(hook) && actions.get(hook).contains(callback));
    }
    
    /**
     * removed the given callback from the given hook.  Returns true if the action was removed
     * @param hook
     * @param callback
     * @return
     */
    ...
    private static void invoke_callback(Runnable callback, Object[] args) {
        ...
        switch (args.length) {
            ...
            case 4:
                ...
                break;
            default:
                WebServer.triggerPluggableError("[invoke_callback] invoking callbacks with more than 4 arguments is not supported");
        }
    }
    ...
    private static Object invoke_callback(Callable callback, Object[] args) {
        switch (args.length) {
            ...
            case 1:
                if (! (callback instanceof Callable1Arg)) return null;
                return ((Callable1Arg) callback).call(args[0]);                
            case 2:
                if (! (callback instanceof Callable2Args)) return null;
                return ((Callable2Args) callback).call(args[0], args[1]);
            case 3:
                if (! (callback instanceof Callable3Args)) return null;
                return ((Callable3Args) callback).call(args[0], args[1], args[2]);
            case 4:
                if (! (callback instanceof Callable4Args)) return null;
                return ((Callable4Args) callback).call(args[0], args[1], args[2], args[3]);
            default:
                WebServer.triggerPluggableError("[invoke_callback] invoking callbacks with more than 4 arguments is not supported");
        }
        return null;
